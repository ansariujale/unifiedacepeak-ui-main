...
const Login = () => {
  ...
  const { mutate: mutateLogin, isPending: isLoginPending } = useMutation({
    mutationFn: login,
    onSuccess: (data) => {
      console.log('[LOGIN DEBUG] onSuccess fired, data:', JSON.stringify(data?.data).substring(0, 200));
      setCaptchaToken(null);
      turnstileRef.current?.reset();
      const { auth, token } = getAuthResponseData(data);
      console.log('[LOGIN DEBUG] auth email:', auth?.email, 'token:', token ? 'present' : 'absent');
      ...
      console.log('[LOGIN DEBUG] about to send OTP for:', email);
      if (email) mutateSendOtp({ email, device_id: getDeviceId() });
      else console.log('[LOGIN DEBUG] NO EMAIL FOUND - OTP not sent');
    },
    onError: (err: any) => {
      console.log('[LOGIN DEBUG] onError fired:', err?.message, err?.response?.status);
